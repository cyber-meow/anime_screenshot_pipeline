...
def caption_add_content(
        caption, info_dict, attribute,
        prob, to_text, sep_string):
    if random.random() >= prob or attribute not in info_dict:
        return caption
    to_add = info_dict[attribute]
    if not to_add:
        return caption
    to_add_text = to_text(to_add, sep_string)
...
def to_text_npeople(count, sep_string):
    suffix = 'person' if count == 1 else 'people'
    return f'{count}{suffix}'


def to_text_characters(to_add, sep_string):
...
def to_text_copyright(to_add, sep_sttring):
    if not isinstance(to_add, list):
        to_add = [to_add]
    to_add = list(filter(lambda item: item != 'unknown', to_add))
...
def to_text_type(image_type, sep_string):
    return image_type


def to_text_artist(to_add, sep_string):
    if not isinstance(to_add, list):
        to_add = [to_add]
...
def to_text_rating(to_add, sep_string):
    if to_add == 'explicit':
        return 'explicit'
    else:
        return None
...
def dict_to_caption(info_dict, args):
    ...
    for attribute, prob in zip(_CAPTIONING_METHODS.keys(), use_probs):
        caption = caption_add_content(
            caption, info_dict, attribute, prob,
    ...
    if 'processed_tags' in info_dict:
        ...
        caption = caption_add_content(
            caption, info_dict, 'tags', args.use_tags_prob,
            to_text_tags, sep_string)
    if args.caption_no_underscore:
