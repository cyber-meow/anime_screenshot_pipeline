...
def setup_logging(log_dir, log_prefix):
    ...
    if log_dir.lower() != 'none':
        ...
        log_file = os.path.join(log_dir,
                                f"{log_prefix}_{str_current_time}.log")
...
def extract_frames(args, src_dir, is_start_stage):
    dst_dir = os.path.join(args.dst_dir, 'intermediate', args.image_type,
                           'raw')
    os.makedirs(dst_dir, exist_ok=True)
    logging.info(f'Extracting frames to {dst_dir} ...')
    extract_and_remove_similar(src_dir,
                               dst_dir,
                               args.image_prefix,
...
def crop_characters(args, src_dir, is_start_stage):
    ...
    if args.crop_with_head:
        source = source.attach(MinHeadCountAction(1, level='n'), )
    if args.crop_with_face:
        source = source.attach(MinFaceCountAction(1, level='n'), )
...
def classify_characters(args, src_dir, is_start_stage):
    ...
    if args.character_ref_dir is None:
        ...
        cluster_from_directory(src_dir,
                               dst_dir,
        ...
        classify_from_directory(src_dir,
                                dst_dir,
...
def select_images_for_dataset(args, src_dir, is_start_stage):
    ...
    update_trigger_word_info(trigger_word_filepath, characters,
    ...
    if args.use_3stage_crop:
        ...
        source.attach(ThreeStageSplitAction(split_person=False), ).export(
            SaveExporter(classified_dir, in_place=True))

    # select images, resize, and save to training
    resize_character_images([classified_dir, full_dir],
                            dst_dir,
...
def tag_and_caption(args, src_dir, is_start_stage):
    ...
    if 'character' in args.pruned_mode:
        ...
        char_tag_proc = CharacterTagProcessor(drop_difficulty,
                                              emb_init_difficutly=0)
    dst_dir = os.path.join(args.dst_dir, 'training', args.image_type)
    ...
    if args.pruned_mode == 'character_core':
        ...
        if args.use_existing_core_tag_file:
            ...
            character_core_tags = char_tag_proc.categorize_character_tag_dict(
                character_core_tags)
            save_core_tag_info(character_core_tags, core_tag_path,
                               wildcard_path)
        source = source.attach(
            CoreCharacterTagPruningAction(character_core_tags,
                                          tags_attribute='processed_tags'))
    ...
    if is_start_stage and args.load_aux:
        ...
        source.export(
            SaveExporter(src_dir,
                         ...
                         in_place=True))
        rearrange_related_files(src_dir)
    arrange_folder(src_dir, src_dir, args.arrange_format,
...
def balance(args, src_dir, is_start_stage):
    ...
    get_repeat(training_dir, weight_mapping, args.min_multiply,
               args.max_multiply, log_file)
    return training_dir
...
if __name__ == "__main__":
    ...
    parser.add_argument("--start_stage",
                        default="1",
                        help="Stage or alias to start from")
    parser.add_argument("--end_stage",
                        default="4",
    ...
    parser.add_argument("--no_remove_similar",
                        action="store_true",
    ...
    parser.add_argument("--image_save_ext",
                        default='.webp',
                        help="Dataset image extensino")
    parser.add_argument("--n_anime_reg",
                        type=int,
                        default=500,
    ...
    parser.add_argument("--caption_no_underscore",
                        action="store_true",
    ...
    parser.add_argument("--use_artist_prob",
                        type=float,
                        default=0,
                        help="Probability to include artist info in captions")
    parser.add_argument("--use_rating_prob",
                        type=float,
                        default=1,
                        help="Probability to include rating info in captions")
    parser.add_argument("--use_tags_prob",
                        type=float,
                        default=1,
    ...
    parser.add_argument("--max_character_number",
                        type=int,
                        default=6,
    ...
    # For balancing
    parser.add_argument('--min_multiply',
                        type=float,
                        default=1,
                        help='Minimum multiply of each image')
    parser.add_argument('--max_multiply',
                        type=int,
                        default=100,
