...
def detect_face(detector, image):
    ...
    for i in range(len(preds)):
        ...
        if (score > max_score
                and max_score < SCORE_THRES) or (score >= SCORE_THRES
                                                 and size > max_size):
            index = i
            max_score = score
            max_size = size
...
def process(args):
    ...
    if args.crop_size is None:
        ...
        assert len(
            tokens
        ) == 2, f"crop_size must be 'width,height' / crop_sizeは'幅,高さ'で指定してください"
    ...
    if args.crop_ratio is None:
        ...
        assert len(
            tokens
        ) == 2, f"crop_ratio must be 'horizontal,vertical' / crop_ratioは'幅,高さ'の倍率で指定してください"
    ...
    for path in tqdm(paths):
        ...
        if crop_width is not None or crop_h_ratio is not None:
            ...
            if scale != 1.0:
                ...
                image = cv2.resize(image, (w, h),
                                   ...
                                   if scale < 1.0 else cv2.INTER_LANCZOS4)
                cx = int(cx * scale + .5)
                cy = int(cy * scale + .5)
                fw = int(fw * scale + .5)
                fh = int(fh * scale + .5)
        ...
        # debug
        if args.debug:
            cv2.rectangle(image, (cx - fw // 2, cy - fh // 2),
                          (cx + fw // 2, cy + fh // 2), (255, 0, 255),
                          fw // 20)
        ...
        with open(
                os.path.join(
                    args.dst_dir,
                    f"{basename}_{cx:04d}_{cy:04d}_{fw:04d}_{fh:04d}{output_extension}"
                ), "wb") as f:
            buf.tofile(f)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--src_dir",
                        type=str,
                        help="directory to load images / 画像を読み込むディレクトリ")
    parser.add_argument("--dst_dir",
                        type=str,
                        help="directory to save images / 画像を保存するディレクトリ")
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=
        "resize to fit smaller side after cropping / 切り出し後の画像の短辺がcrop_sizeにあうようにリサイズする"
    )
    parser.add_argument(
        "--resize_face_size",
        type=int,
        default=None,
        help=
        "resize image before cropping by face size / 切り出し前に顔がこのサイズになるようにリサイズする"
    )
    parser.add_argument(
        "--crop_size",
        type=str,
        default=None,
        help=
        "crop images with 'width,height' pixels, face centered / 顔を中心として'幅,高さ'のサイズで切り出す"
    )
    parser.add_argument(
        "--crop_ratio",
        type=str,
        default=None,
        help=
        "crop images with 'horizontal,vertical' ratio to face, face centered / 顔を中心として顔サイズの'幅倍率,高さ倍率'のサイズで切り出す"
    )
    parser.add_argument("--debug",
                        action="store_true",
                        help="render rect for face / 処理後画像の顔位置に矩形を描画します")
