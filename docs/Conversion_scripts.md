...
The script [batch_hcp_convert.py](https://github.com/cyber-meow/anime_screenshot_pipeline/blob/main/utilities/batch_hcp_convert.py) performs conversion between hcp and sd-webui format. It applies to LoRA, LyCORIS, and full model (for full model only conversion from hcp to webui is implemented).
...
The script [batch_bundle_convert.py](https://github.com/cyber-meow/anime_screenshot_pipeline/blob/main/utilities/batch_bundle_convert.py) performs conversion between bundle and non-bundle format (i.e., embeddings saved separately).
