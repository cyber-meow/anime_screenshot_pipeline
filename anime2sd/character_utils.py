...
def parse_ref_dir(ref_dir):
    ...
    subdirs = [d for d in os.listdir(
        ref_dir) if os.path.isdir(os.path.join(ref_dir, d))]
    if subdirs:
        for subdir in subdirs:
            ...
            for filename in os.listdir(os.path.join(ref_dir, subdir)):
                if os.path.splitext(filename)[1].lower() in image_extensions:
                    ref_image_files.append(
                        os.path.join(ref_dir, subdir, filename))
...
def classify_characters_map_clusters(
        imgs, ref_labels, cluster_ids,
        mode='min', same_threshold=0.9,
        ref_images=None, batch_diff=None, batch_same=None, desc=None):
    ...
    for cluster_id in unique_clusters:
        cluster_indices = [idx for idx, img in enumerate(
            imgs) if cluster_ids[idx] == cluster_id]

        if batch_diff is not None:
            avg_cluster_diff = np.mean([batch_diff[idx]
                                       for idx in cluster_indices], axis=0)
        else:
            cluster_imgs = [imgs[idx] for idx in cluster_indices]
            avg_cluster_diff = np.mean([np.array([ccip_difference(
                img, ref_img) for ref_img in ref_images])
                for img in cluster_imgs], axis=0)

        if mode == 'avg':
            avg_dists = np.array([
                avg_cluster_diff[ref_labels == label].mean() if len(
                    avg_cluster_diff[ref_labels == label]) > 0 else np.inf
                for label in range(0, max_label + 1)
            ])
        elif mode == 'min':
            avg_dists = np.array([
                avg_cluster_diff[ref_labels == label].min() if len(
                    avg_cluster_diff[ref_labels == label]) > 0 else np.inf
                for label in range(0, max_label + 1)
            ])
        else:
...
def classify_characters(
        imgs,
        same_threshold=0.9, default_thresohold_ratio=0.75,
        ref_labels=None, cluster_labels=None,
        ref_images=None, batch_diff=None, batch_same=None, desc=None):

    cls_labels = -1 * np.ones(len(imgs))
    max_ref_label = np.max(
        ref_labels) if ref_labels is not None else -1
    max_cluster_label = np.max(
        cluster_labels) if cluster_labels is not None else -1

    for i, img in tqdm(enumerate(imgs), desc=desc):
        if ref_images is not None:
            ref_diff = np.array([ccip_difference(img, ref_img)
                                for ref_img in ref_images])
            ref_avg_dists = np.array([
                ref_diff[ref_labels == label].min() if len(
                    ref_diff[ref_labels == label]) > 0 else np.inf
        ...
        if batch_diff is not None:
            ...
            cluster_avg_dists = np.array([
                batch_diff_i[cluster_labels == label].mean() if len(
                    batch_diff_i[cluster_labels == label]) > 0 else np.inf
...
def extract_from_noise(images, labels,
                       batch_diff, batch_same,
                       ref_images=None, ref_labels=None):
    ...
    label_cnt = {i: (labels == i).sum()
                 for i in range(-1, max(labels)+1) if (labels == i).sum() > 0}
    logging.info(f'Current label count: {label_cnt}')


def merge_clusters(exist_ids, max_clu_id, batch_same, labels,
    ...
    while True:
        ...
        for xi in range(0, max_clu_id + 1):
            ...
            for yi in range(xi + 1, max_clu_id + 1):
                ...
                logging.info(
                    f'Label {xi} and {yi}\'s similarity score: {score}')
    ...
    logging.info('Merge complete, remained cluster ids: '
                 + f'{sorted(exist_ids)}.')
    label_cnt = {i: (labels == i).sum()
                 for i in range(-1, max_clu_id+1) if (labels == i).sum() > 0}
    logging.info(f'Current label count: {label_cnt}')


def cluster_characters(images,
    ...
    clustering = OPTICS(min_samples=clu_min_samples,
                        metric=_metric).fit(samples)
    labels = clustering.labels_.astype(int)

    max_clu_id = labels.max().item()
    all_label_ids = np.array([-1, *range(0, max_clu_id + 1)])
    logging.info(
        f'Cluster complete, with {plural_word(max_clu_id, "cluster")}.')
    label_cnt = {i: (labels == i).sum()
                 for i in all_label_ids if (labels == i).sum() > 0}
    logging.info(f'Current label count: {label_cnt}')
    ...
    if to_merge_clusters:
        ...
        merge_clusters(_exist_ids, max_clu_id, batch_same, labels,
                       merge_threshold=0.85)
...
def classify_from_directory(src_dir, dst_dir, ref_dir,
    ...
    ref_images = np.array([ccip_extract_feature(img) for img in tqdm(
        ref_image_files, desc='Extract reference features')])
    clu_ids[clu_ids >= 0] += max(ref_labels) + 1
    if to_extract_from_noise:
        extract_from_noise(images,
                           labels=clu_ids,
                           batch_diff=batch_diff, batch_same=batch_same,
                           ref_images=ref_images, ref_labels=ref_labels)
