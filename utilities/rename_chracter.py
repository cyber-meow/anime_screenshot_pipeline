"""Mostly written by chatgpt but the orignal version had some bug"""
...
def rename_folder(folder_name, class_mapping, keep_unknown_class=False):
    ...
    if unknown_class and not keep_unknown_class:
        return None
...
def rename_folder_and_tags(folder, class_mapping, keep_unknown_class=False):
    new_folder_name = rename_folder(folder, class_mapping, keep_unknown_class)
    ...
    for file in os.listdir(new_folder_name):
        if file.endswith(('.png', '.jpg', 'jpeg', 'webm')):
            tags_file = os.path.join(new_folder_name, file + '.tags')
        if os.path.exists(tags_file):
            modify_tags_file(tags_file, class_mapping)


def main(src_dir, class_mapping_csv, keep_unknown_class):
    ...
    for folder in tqdm(os.listdir(src_dir)):
        rename_folder_and_tags(os.path.join(
            src_dir, folder), class_mapping, keep_unknown_class)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--keep_unknown_class', action='store_true',
                        help='Keep folders with unknown class names')
    args = parser.parse_args()
    main(args.src_dir, args.class_mapping_csv, args.keep_unknown_class)
